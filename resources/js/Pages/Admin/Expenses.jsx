...
export default function Expenses({ expenses, accounts }) {
    ...
    const handleDelete = (id) => {
        setConfirmModal({
            ...
            message: 'Apakah Anda yakin ingin menghapus catatan pengeluaran ini? Jurnal terkait akan dibatalkan.',
            type: 'danger',
            confirmText: 'Hapus',
            onConfirm: () => {
        ...
        <AdminLayout title="Pengeluaran Operasional">
            ...
            <div className="p-4 max-w-7xl mx-auto space-y-6 text-left">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-2xl font-normal tracking-tight" style={{ color: 'var(--g-text-primary)' }}>Pengeluaran Operasional</h1>
                        <p className="text-sm mt-1" style={{ color: 'var(--g-text-muted)' }}>Catat dan kelola pengeluaran bisnis secara manual</p>
                    </div>
                    <button
                        onClick={() => openModal()}
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-white/5">
                                {expenses.data.length === 0 && (
                                    <tr>
                                        <td colSpan="5" className="p-12 text-center text-xs opacity-20 uppercase font-bold tracking-widest">Tidak ada catatan pengeluaran ditemukan</td>
                                    </tr>
                                )}
                                {expenses.data.map((expense) => (
                                    <tr key={expense.id} className="hover:bg-white/5 transition-colors group">
                                        <td className="px-4 py-4 whitespace-nowrap text-white/40 font-mono text-xs">{new Date(expense.date).toLocaleDateString('id-ID')}</td>
                                        <td className="px-4 py-4">
                                            <div className="font-bold text-sm" style={{ color: 'var(--g-text-primary)' }}>{expense.category}</div>
                                            <div className="text-[9px] opacity-40 uppercase font-bold tracking-widest mt-0.5" style={{ color: 'var(--g-text-muted)' }}>Beban Bisnis</div>
                                        </td>
                                        <td className="px-4 py-4 text-white/50 max-w-xs truncate">{expense.description || '-'}</td>
                                        <td className="px-4 py-4 text-right">
                                            <span className="text-red-400 font-mono font-bold">-{fmt(expense.amount)}</span>
                                        </td>
                                        <td className="px-4 py-4">
                                            <div className="flex justify-center gap-2">
                                                <button onClick={() => openModal(expense)} className="p-2 rounded-lg bg-white/5 text-white/40 hover:bg-white/10 hover:text-blue-400 transition-all">
                                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                </button>
                                                <button onClick={() => handleDelete(expense.id)} className="p-2 rounded-lg bg-white/5 text-white/40 hover:bg-white/10 hover:text-red-400 transition-all">
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                    <div className="w-full max-w-lg rounded-lg border shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200" style={{ backgroundColor: 'var(--g-bg-secondary)', borderColor: 'var(--g-border)' }}>
                        <div className="px-4 py-4 border-b bg-black/5 flex justify-between items-center" style={{ borderColor: 'var(--g-border)' }}>
                            <div>
                                <h3 className="text-sm font-bold uppercase tracking-widest opacity-60" style={{ color: 'var(--g-text-tertiary)' }}>{editingExpense ? 'Edit Pengeluaran' : 'Pengeluaran Baru'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-4 space-y-5">
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    placeholder="Contoh: Pembayaran Listrik, ATK Kantor"
                                    value={data.category}
                            ...
                            <div>
                                <label className="block text-[10px] uppercase font-bold text-white/40 mb-1.5 tracking-widest">Akun Target (Debit)</label>
                                <select
                                    ...
                                    style={{ backgroundColor: 'var(--g-input-bg)', borderColor: 'var(--g-input-border)', color: 'var(--g-text-primary)' }}
                                >
                                    <option value="" className="bg-[#1A1A1A]">Pilih Akun Beban</option>
                                    {accounts.map(acc => (
